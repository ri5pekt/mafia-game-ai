...
import { DEFAULT_API_BASE, appendGameEvent, connectGameSse, createGame, endGame, getActiveGame, getGameEvents, getGameMeta } from '@/game/api';
import { buildChatMessages } from '@/game/chat';

type CreatePayload = { players: { id: string; seatNumber: number; name: string; nickname: string }[]; host: { id: string; name: string; nickname: string } };
...
export function useGameSession(opts: {
  ...
  async function createNewGame() {
    ...
    try {
      ...
      if (!gameEvents.value.some((ev) => ev.type === 'HOST_MESSAGE' && ev.payload?.tag === 'DISCUSSION_START')) {
        ...
        const text = first ? `Day 1 discussion. ${first.name} (${first.nickname}) will speak first.` : 'Day 1 discussion begins.';
  ...
  async function pumpHostBubbleQueue() {
    ...
    showBubble('host', item.text, item.ms);
    window.setTimeout(() => {
  ...
  async function onVote() {
    ...
    await appendEvent({ type: 'PLAYER_VOTE', kind: 'player', payload: { voterSeatNumber: voter, targetSeatNumber: voteSelection.value } });
  ...
  async function onVoteTurnEnded() {
    ...
    if (top.length === 1) {
      const target = top[0];
      await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: `${seatLabel(target)} is eliminated.` } });
      await appendEvent({ type: 'PLAYER_ELIMINATED', kind: 'system', payload: { seatNumber: target, reason: 'VOTE' } });
      await appendEvent({ type: 'PHASE_CHANGED', kind: 'system', payload: { from: phase, to: 'ELIMINATION_SPEECH', eliminated: [target] } });
      await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: `Final words: ${seatLabel(target)}.` } });
      return;
    }
    ...
    if (phase === 'DAY_VOTING') {
      await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: `Tie between: ${names}. They will speak, then we revote.` } });
      await appendEvent({ type: 'PHASE_CHANGED', kind: 'system', payload: { from: phase, to: 'TIE_DISCUSSION', candidates: top } });
      await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: `Tie discussion begins. ${seatLabelWithNick(top[0])} will speak.` } });
      return;
    }
    ...
    await appendEvent({
      type: 'HOST_MESSAGE',
      kind: 'host',
      payload: { text: `Still tied after revote (${max} votes): ${names}. Mass elimination proposal begins (vote YES/NO).` }
    });
    await appendEvent({ type: 'PHASE_CHANGED', kind: 'system', payload: { from: phase, to: 'MASS_ELIMINATION_PROPOSAL', candidates: top } });
    const firstVoter = aliveSeatNumbers.value[0] ?? 1;
    await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: `Seat #${firstVoter}, vote YES/NO.` } });
    return;
  }
  ...
  async function onProposalVote() {
    ...
    const voter = loopState.value.currentSpeakerSeatNumber;
    await appendEvent({ type: 'MASS_ELIMINATION_VOTE', kind: 'system', payload: { voterSeatNumber: voter, vote: yesNoSelection.value } });
    ...
    if (yes > alive.length / 2) {
      ...
      await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: `Majority YES. Eliminating: ${names}.` } });
      for (const c of candidates) {
        await appendEvent({ type: 'PLAYER_ELIMINATED', kind: 'system', payload: { seatNumber: c, reason: 'MASS_ELIMINATION' } });
      }
      await appendEvent({
        type: 'PHASE_CHANGED',
        kind: 'system',
      ...
      await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: `Final words: ${seatLabel(first)}.` } });
      return;
    }

    await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: 'Majority NO. No one is eliminated. Night falls.' } });
    const mafia = mafiaSeatNumbers.value;
    await appendEvent({
      type: 'PHASE_CHANGED',
      kind: 'system',
  ...
  async function resolveNightAndStartDay() {
    ...
    await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: 'Morning comes. Everyone wakes up.' } });
    await sleep(750);

    // 2) Flip to a non-night phase so everyone visually wakes up (unmask).
    await appendEvent({ type: 'PHASE_CHANGED', kind: 'system', payload: { from: nightPhase, to: 'MORNING_REVEAL' } });
    await sleep(750);

    // 3) Announce outcome, then transition to next phase.
    if (kill != null && aliveSet.value.has(kill)) {
      await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: `${seatLabel(kill)} was killed during the night.` } });
      await sleep(650);
      await appendEvent({ type: 'PLAYER_ELIMINATED', kind: 'system', payload: { seatNumber: kill, reason: 'MAFIA' } });
      await appendEvent({ type: 'PHASE_CHANGED', kind: 'system', payload: { from: 'MORNING_REVEAL', to: 'ELIMINATION_SPEECH', eliminated: [kill] } });
      await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: `Final words: ${seatLabel(kill)}.` } });
      return;
    }

    await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: 'No one was killed tonight.' } });
    await sleep(650);
    await appendEvent({ type: 'PHASE_CHANGED', kind: 'system', payload: { from: 'MORNING_REVEAL', to: 'DAY_DISCUSSION' } });
    await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: 'Day discussion begins.' } });
  }
  ...
  async function checkWinAndMaybeEnd() {
    ...
    if (mafia === 0) {
      ...
      await appendEvent({ type: 'PHASE_CHANGED', kind: 'system', payload: { from: 'WIN_CHECK', to: 'GAME_END' } });
      return;
    }
    if (mafia >= town) {
  ...
  const nightGuessOptions = computed(() => aliveSeatNumbers.value.map((s) => ({ label: `#${s} ${seatLabel(s)}`, value: s })));

  function canActAsBossNow(): boolean {
    return loopState.value.phaseId === 'NIGHT_MAFIA_DISCUSSION' && loopState.value.currentSpeakerSeatNumber === actingBossSeatNumber.value;
  }
  ...
  async function onNightSelectKillTarget(targetSeatNumber: number) {
    ...
    await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: `Kill target selected: ${seatLabel(targetSeatNumber)}.` } });
  }
  ...
  async function onBossGuess(targetSeatNumber: number) {
    ...
    await appendEvent({ type: 'NIGHT_BOSS_GUESS', kind: 'player', payload: { seatNumber: boss, targetSeatNumber, isSheriff } });
    await appendEvent({
      type: 'HOST_MESSAGE',
      kind: 'host',
      payload: { text: isSheriff ? `${seatLabel(targetSeatNumber)} is the Sheriff.` : `${seatLabel(targetSeatNumber)} is NOT the Sheriff.` }
    });
    ...
    await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: `${seatLabel(sheriff)} (Sheriff) is awake. Choose someone to investigate.` } });
  }
  ...
  async function onSheriffInvestigate(targetSeatNumber: number) {
    ...
    await appendEvent({ type: 'NIGHT_SHERIFF_INVESTIGATE', kind: 'player', payload: { seatNumber: sheriff, targetSeatNumber, isMafia } });
    await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: isMafia ? `${seatLabel(targetSeatNumber)} is MAFIA.` : `${seatLabel(targetSeatNumber)} is TOWN.` } });
    await resolveNightAndStartDay();
  }
  ...
  function defaultPhasePayload(to: LocalPhaseId) {
    ...
    if (to === 'NIGHT_MAFIA_BOSS_GUESS') return { from, to, speakers: actingBossSeatNumber.value != null ? [actingBossSeatNumber.value] : [] };
    if (to === 'NIGHT_SHERIFF_ACTION') return { from, to, speakers: sheriffSeatNumber.value != null ? [sheriffSeatNumber.value] : [] };
    if (to === 'DAY_VOTING') return { from, to, candidates: nominees.value };
    if (to === 'TIE_DISCUSSION' || to === 'TIE_REVOTE' || to === 'MASS_ELIMINATION_PROPOSAL') return { from, to, candidates: tieCandidates.value };
  ...
  async function devSwitchPhase() {
    ...
    if (to === 'NIGHT_MAFIA_DISCUSSION') {
      const speakers: number[] = Array.isArray((payload as any).speakers) ? (payload as any).speakers : bossLastOrder(mafiaSeatNumbers.value);
  ...
  async function onEndTurn() {
    ...
    if (phase === 'DAY_DISCUSSION') {
      ...
      if (nominees.value.length === 0) {
        await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: 'No one was nominated today. Moving to the night phase.' } });
        const mafia = mafiaSeatNumbers.value;
        await appendEvent({
          type: 'PHASE_CHANGED',
          kind: 'system',
      ...
      await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: `Voting begins. Candidates: ${list}.` } });
      await appendEvent({
        type: 'PHASE_CHANGED',
        kind: 'system',
      ...
      await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: `Seat #${firstVoter}, please vote.` } });
      return;
    }
    ...
    if (phase === 'TIE_DISCUSSION') {
      ...
      if (!isLast) {
        ...
        await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: `Next: ${seatLabel(nextSeat)} will speak.` } });
        return;
      }
      await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: 'Tie discussion complete. Revote begins.' } });
      await appendEvent({ type: 'PHASE_CHANGED', kind: 'system', payload: { from: 'TIE_DISCUSSION', to: 'TIE_REVOTE', candidates: c } });
      const firstVoter = aliveSeatNumbers.value[0] ?? 1;
      await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: `Seat #${firstVoter}, please vote.` } });
      return;
    }
    ...
    if (phase === 'ELIMINATION_SPEECH') {
      ...
      if (!isLast) {
        ...
        await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: `Final words: ${seatLabel(nextSeat)}.` } });
        return;
      }
      await appendEvent({ type: 'PHASE_CHANGED', kind: 'system', payload: { from: 'ELIMINATION_SPEECH', to: 'WIN_CHECK' } });
      ...
      await appendEvent({ type: 'PHASE_CHANGED', kind: 'system', payload: { from: 'WIN_CHECK', to: 'NIGHT_MAFIA_DISCUSSION', speakers: mafia } });
      await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: 'Night falls. Mafia wake up.' } });
      return;
    }
    ...
    if (phase === 'NIGHT_MAFIA_DISCUSSION') {
      ...
      if (!isLast) {
        ...
        await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: `Next mafia: ${seatLabel(nextSeat)} will speak.` } });
        return;
      }

      const bossActor = actingBossSeatNumber.value;
      if (bossActor == null) {
        await appendEvent({ type: 'PHASE_CHANGED', kind: 'system', payload: { from: 'NIGHT_MAFIA_DISCUSSION', to: 'WIN_CHECK' } });
        await checkWinAndMaybeEnd();
        return;
      }
      await appendEvent({
      ...
      await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: `${seatLabel(bossActor)} (boss) is awake. Choose someone to check for Sheriff.` } });
      return;
    }
    ...
    if (phase === 'NIGHT_MAFIA_BOSS_GUESS') {
      ...
      await appendEvent({ type: 'PHASE_CHANGED', kind: 'system', payload: { from: 'NIGHT_MAFIA_BOSS_GUESS', to: 'NIGHT_SHERIFF_ACTION', speakers: [sheriff] } });
      await appendEvent({ type: 'HOST_MESSAGE', kind: 'host', payload: { text: `${seatLabel(sheriff)} (Sheriff) is awake. Choose someone to investigate.` } });
      return;
    }
